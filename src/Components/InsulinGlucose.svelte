...
<h1 class="body-header">Glucose vs Insulin For those With and Without Diabetes</h1>
<p class="body-text">
...
<div class="svg-container">
  <svg width={width} height={height}>
    ...
    <g transform={`translate(${margin.left}, 10)`}>
      ...
      {/each}
      <text x={-350} y={-55} text-anchor="middle" transform="rotate(-90)" class="axis-label">
    ...
    <g class="voronoi">
      {#each diabetesData as data, i}
        ...
        <path 
          ...
          fill="none" 
          pointer-events="all"
          on:mouseenter={event => showTooltip(event, diabetesData[i])}
          on:mouseleave={hideTooltip}
        />
      {/each}
    </g>
